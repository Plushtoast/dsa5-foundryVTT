                    ...
                    callback: async (event, button, dialog) => {
                        ...
                        if (pending?.effectUuid) {
                            ...
                            if (diesToReroll.length > maxDice) {
                                ui.notifications.warn(game.i18n.format('DIALOG.postRollRerollMaxDice', { max: maxDice }), { permanent: false });
                                return;
                            }
                        }
                        ...
                        if (pendingAfter?.effectUuid) {
                            try {
                                ...
                                const extra = game.i18n.format('ActiveEffects.chargesChatPostRollRerolled', { count: diceCount });

                                const effectName = effect?.name || effect?.label || game.i18n.localize('ActiveEffects.custom');
                                ...
                                const rollLine = `<p><b>${localize('Roll')}</b>: ${changedRolls.join(', ')}</p>`;
