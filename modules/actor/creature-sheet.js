...
export default class ActorSheetdsa5Creature extends ActorSheetDsa5 {
  ...
  get template() {
    if (this.showLimited())
      return 'systems/dsa5/templates/actors/creature-limited.html';
  ...
  async getData(options) {
    ...
    data.enrichedDescription = await TextEditor.enrichHTML(
      getProperty(this.actor.system, 'description.value'),
      { secrets: this.object.isOwner, async: true },
    );
    data.enrichedBehaviour = await TextEditor.enrichHTML(
      getProperty(this.actor.system, 'behaviour.value'),
      { secrets: this.object.isOwner, async: true },
    );
    data.enrichedFlight = await TextEditor.enrichHTML(
      getProperty(this.actor.system, 'flight.value'),
      { secrets: this.object.isOwner, async: true },
    );
    data.enrichedSpecialrules = await TextEditor.enrichHTML(
      getProperty(this.actor.system, 'specialRules.value'),
      { secrets: this.object.isOwner, async: true },
    );
    return data;
  }
  ...
  async _addTrait(item) {
    let res = this.actor.items.find(
      (i) => i.type == 'trait' && i.name == item.name,
    );
    if (!res) {
