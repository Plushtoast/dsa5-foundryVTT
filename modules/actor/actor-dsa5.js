...
export default class Actordsa5 extends Actor {
  ...
  static async create(data, options) {
    if (data instanceof Array || data.items)
    ...
    if (data.type != 'character')
      data.system = { status: { fatePoints: { current: 0, value: 0 } } };

    if (
      data.type != 'creature' &&
      [undefined, 0].includes(getProperty(data, 'system.status.wounds.value'))
    )
  ...
  _getArmorCompensation(actor, wornArmors, itemModifiers) {
    ...
    if (armorCompensation > armorEncumbrance) {
      const modKeys = [
        game.i18n.localize('CHARAbbrev.GS'),
        game.i18n.localize('CHARAbbrev.INI'),
      ];
      for (let modkey of modKeys) {
        if (!itemModifiers[modkey]) continue;

        itemModifiers[modkey] = itemModifiers[modkey].filter(
          (x) => x.type != 'armor',
        );
      }
    }
  ...
  _getItemModifiers() {
    ...
    for (let i of this.items.filter(
      (x) =>
        (['meleeweapon', 'rangeweapon', 'armor', 'equipment'].includes(
          x.type,
        ) &&
          getProperty(x, 'system.worn.value')) ||
  ...
  prepareDerivedData() {
    ...
    try {
      ...
      for (let ch of Object.values(data.characteristics)) {
        ch.value =
          ch.initial + ch.advances + (ch.modifier || 0) + ch.gearmodifier;
      }
      ...
      for (const i of this.items) {
        if (moneyHasWeight && i.type == 'money') {
          i.system.preparedWeight = parseFloat(
            (i.system.weight.value * i.system.quantity.value).toFixed(3),
          );
          ...
          if (i.type == 'armor') {
            ...
            data.totalWeight += parseFloat(
              (
                ...
                (i.system.worn.value
                  ? Math.max(0, i.system.quantity.value - 1)
          ...
          switch (i.type) {
            ...
            case 'blessing':
              ...
              break;
            case 'specialability':
              if (DSA5.sortedSpecs.magical.has(i.system.category.value))
                data.isMage = true;
              else if (DSA5.sortedSpecs.clerical.has(i.system.category.value))
                data.isPriest = true;
              break;
          }
        }
      ...
      data.canAdvance =
        this.isOwner &&
        (this.type == 'character' || data.isFamiliar || data.isPet);
      this.canAdvance = data.canAdvance;

      data.carrycapacity =
        data.characteristics.kk.value * 2 + data.carryModifier;

      if (data.canAdvance) {
        data.details.experience.current =
          data.details.experience.total - data.details.experience.spent;
        data.details.experience.description =
          DSA5_Utility.experienceDescription(data.details.experience.total);
      }
      if (this.type == 'character' || this.type == 'npc') {
        data.status.wounds.current =
          data.status.wounds.initial + data.characteristics.ko.value * 2;
        data.status.soulpower.value =
          (data.status.soulpower.initial || 0) +
          Math.round(
            (data.characteristics.mu.value +
              data.characteristics.kl.value +
              data.characteristics.in.value) /
              6,
          );
        data.status.toughness.value =
          (data.status.toughness.initial || 0) +
          Math.round(
            (data.characteristics.ko.value +
              data.characteristics.ko.value +
              data.characteristics.kk.value) /
              6,
          );
      ...
      data.status.fatePoints.max =
        Number(data.status.fatePoints.current) +
      ...
      data.status.wounds.max = Math.round(
        (data.status.wounds.current +
          data.status.wounds.modifier +
          data.status.wounds.advances) *
          data.status.wounds.multiplier +
          data.status.wounds.gearmodifier,
      );

      data.status.regeneration.LePmax =
        data.status.regeneration.LePTemp +
        data.status.regeneration.LePMod +
        data.status.regeneration.LePgearmodifier;
      data.status.regeneration.KaPmax =
        data.status.regeneration.KaPTemp +
        data.status.regeneration.KaPMod +
        data.status.regeneration.KaPgearmodifier;
      data.status.regeneration.AsPmax =
        data.status.regeneration.AsPTemp +
        data.status.regeneration.AsPMod +
      ...
      data.status.astralenergy.permanentLossSum =
        data.status.astralenergy.permanentLoss -
        data.status.astralenergy.rebuy +
        data.status.astralenergy.permanentGear;
      data.status.karmaenergy.permanentLossSum =
        data.status.karmaenergy.permanentLoss -
        data.status.karmaenergy.rebuy +
        data.status.karmaenergy.permanentGear;
      ...
      if (data.isFamiliar || (guide && this.type != 'creature')) {
        ...
        if (data.characteristics[guide.magical])
          data.status.astralenergy.current += Math.round(
            data.characteristics[guide.magical].value *
              data.energyfactor.magical,
          );

        if (data.characteristics[guide.clerical])
          data.status.karmaenergy.current += Math.round(
            data.characteristics[guide.clerical].value *
              data.energyfactor.clerical,
          );
      }
      ...
      data.status.soulpower.max =
        data.status.soulpower.value +
        data.status.soulpower.modifier +
        data.status.soulpower.gearmodifier;
      data.status.toughness.max =
        data.status.toughness.value +
        data.status.toughness.modifier +
        data.status.toughness.gearmodifier;
      data.status.dodge.value =
        Math.round(data.characteristics.ge.value / 2) +
        data.status.dodge.gearmodifier;
      ...
      console.error(
        `Something went wrong with preparing actor data ${this.name}: ` +
          error +
          error.stack,
      );
      ui.notifications.error(
        game.i18n.format('DSAError.PreparationError', { name: this.name }) +
          error +
          error.stack,
      );
    }
  ...
  static async deferredEffectAddition(effect, actor, target) {
    const current =
      actor.effects.find((x) => x.statuses.has(effect))?.flags.dsa5.auto || 0;
    ...
    if (isChange)
      await actor
        .addCondition(effect, target, true, true)
        .then(() => (actor[attr] = undefined));
  }
  ...
  static async postUpdateConditions(actor) {
    ...
    if (
      !TraitRulesDSA5.hasTrait(
        actor,
        game.i18n.localize('LocalizedIDs.painImmunity'),
      )
    ) {
    ...
    if ((actor.type != 'creature' || actor.canAdvance) && !isMerchant) {
      newEncumbrance += Math.max(
        0,
        Math.ceil((data.totalWeight - data.carrycapacity - 4) / 4),
      );
    }
    ...
    if (
      AdvantageRulesDSA5.hasVantage(
        actor,
        game.i18n.localize('LocalizedIDs.blind'),
      )
    )
      await actor.addCondition('blind');
    if (
      AdvantageRulesDSA5.hasVantage(
        actor,
        game.i18n.localize('LocalizedIDs.mute'),
      )
    )
      await actor.addCondition('mute');
    if (
      AdvantageRulesDSA5.hasVantage(
        actor,
        game.i18n.localize('LocalizedIDs.deaf'),
      )
    )
      await actor.addCondition('deaf');
  ...
  prepareSwarm(data) {
    ...
    const effectiveCount = Math.min(
      Math.ceil(data.status.wounds.value / data.swarm.maxwounds),
      count,
    );
    ...
    if (pain < 4)
      pain -=
        AdvantageRulesDSA5.vantageStep(
          this,
          game.i18n.localize('LocalizedIDs.ruggedFighter'),
        ) +
        AdvantageRulesDSA5.vantageStep(
          this,
          game.i18n.localize('LocalizedIDs.ruggedAnimal'),
        ) +
        (SpecialabilityRulesDSA5.hasAbility(
          this,
          game.i18n.localize('LocalizedIDs.traditionKor'),
        )
          ? 1
          : 0);
    if (pain > 0)
      pain +=
        AdvantageRulesDSA5.vantageStep(
          this,
          game.i18n.localize('LocalizedIDs.sensitiveToPain'),
        ) +
        AdvantageRulesDSA5.vantageStep(
          this,
          game.i18n.localize('LocalizedIDs.fragileAnimal'),
        );
  ...
  woundPain(data, attr = 'wounds') {
    ...
    if (data.status[attr].max > 0) {
      const hasDefaultPain =
        this.type != 'creature' || data.status[attr].max >= 20;
      if (hasDefaultPain) {
        pain = Math.floor(
          (1 - data.status[attr].value / data.status[attr].max) * 4,
        );
        if (data.status[attr].value <= 5) pain = 4;
      } else {
        pain = Math.floor(
          5 - (5 * data.status[attr].value) / data.status[attr].max,
        );
      }
    }
  ...
  calcSpeed(data, fixated, horse) {
    if (horse) {
      ...
      data.status.speed.max =
        data.status.speed.initial +
        (data.status.speed.modifier || 0) +
        (data.status.speed.gearmodifier || 0);
      data.status.speed.max = Math.round(
        Math.max(
          0,
          data.status.speed.max - Math.min(4, this.calcEncumbrance(data)),
        ) * data.status.speed.multiplier,
      );

      if (!this.hasCondition('bloodrush'))
        data.status.speed.max = Math.max(
          0,
          data.status.speed.max - (data.condition?.inpain || 0),
        );

      const paralysis = this.hasCondition('paralysed');
      if (paralysis)
        data.status.speed.max = Math.round(
          data.status.speed.max * (1 - paralysis.flags.dsa5.value * 0.25),
        );
      if (fixated) {
        data.status.speed.max = 0;
      } else if (
        this.hasCondition('rooted') ||
  ...
  calcInitiative(data, encumbrance, horse) {
    if (this.type == 'character' || this.type == 'npc') {
      data.status.initiative.value =
        Math.round(
          (data.characteristics.mu.value + data.characteristics.ge.value) / 2,
        ) + (data.status.initiative.modifier || 0);
    } else {
      data.status.initiative.value =
        data.status.initiative.current + (data.status.initiative.modifier || 0);
    }
    ...
    if (horse) {
      ...
      data.status.initiative.value +=
        (data.status.initiative.gearmodifier || 0) - Math.min(4, encumbrance);
      ...
      data.status.initiative.value =
        Math.round(data.status.initiative.value) + baseInit;
    }
  }
  ...
  async prepareMerchant() {
    if (getProperty(this, 'system.merchant.merchantType') == 'loot') {
      if (
        getProperty(this, 'system.merchant.locked') &&
        !this.hasCondition('locked')
      ) {
  ...
  applyActiveEffects() {
    ...
    const appliedArtifacts = this.items
      .filter(
        (x) =>
          ['rangeweapon', 'meleeweapon', 'equipment', 'armor'].includes(
            x.type,
          ) &&
          x.system.isArtifact &&
          (x.system.worn.value ||
            (x.type == 'equipment' && !x.system.worn.wearable)),
      )
      .map((x) => x.system.artifact);
    const disableWeaponAdvantages = !game.settings.get(
      'dsa5',
    ...
    for (let item of this.items) {
      for (const e of item.effects) {
        ...
        switch (item.type) {
          case 'meleeweapon':
          case 'rangeweapon':
            if (disableWeaponAdvantages && e.system.equipmentAdvantage)
              continue;
          ...
          case 'equipment':
            apply =
              !item.system.worn.wearable ||
              (item.system.worn.wearable && item.system.worn.value);
            break;
          case 'trait':
            apply =
              !['meleeAttack', 'rangeAttack'].includes(
                item.system.traitType.value,
              ) || e.getFlag('dsa5', 'applyToOwner');
            ...
            switch (item.system.category.value) {
              ...
              case 'staff':
                apply =
                  ...
                  appliedArtifacts.includes(item.system.artifact);
                break;
              default:
                apply = true;
            }
  ...
  prepareBaseData() {
    ...
    mergeObject(system, {
      ...
      skillModifiers: {
        ...
        ...['liturgy', 'ceremony', 'ritual', 'spell', 'skill'].reduce(
          (prev, x) => {
            prev[x] = {
              FP: [],
              step: [],
          ...
          status: {
            [k]: {
              cost: game.i18n.format('advancementCost', {
                cost: DSA5_Utility._calculateAdvCost(
                  this.system.status[k].advances,
                  'D',
                ),
              }),
    ...
    if (options.origin) {
      ...
      wornArmor = wornArmor.map((armor) => {
            ...
            for (let change of ef.changes) {
              if (change.key == 'self.armorVulnerability') {
                ...
                if (options.defenderTest.attackFromBehind)
                  adaption = adaptions.find((x) =>
                    x.trim().startsWith('attackFromBehind '),
                  );
                if (!adaption)
                  adaption = adaptions.find((x) =>
                    x.trim().startsWith(combatskill),
                  );
                ...
                if (adaption) {
                  ...
                  for (let key of [
                    ...
                    if (optnCopy.armor.system.protection[key])
                      optnCopy.armor.system.protection[key] = Math.max(
                        0,
                        optnCopy.armor.system.protection[key] + number,
                      );
                  }
                } else {
                  adaption = adaptions.find((x) =>
                    x.trim().startsWith('randomArmor '),
                  );
                  if (adaption) {
                    //random value
                    ...
                    const randomArmor =
                      randomArmorVals[
                        Math.floor(Math.random() * randomArmorVals.length)
                      ];
                    ...
                    for (let key of [
                      ...
                      if (optnCopy.armor.system.protection[key])
                        optnCopy.armor.system.protection[key] = randomArmor;
                    }
                  }
                }
              }
        ...
        return DSAActiveEffectConfig.applyRollTransformation(
          actor,
          optnCopy,
          DSATriggers.EVENTS.ARMOR_TRANSFORMATION,
        ).options.armor;
      });
    }
    return wornArmor;
  ...
  static armorValue(actor, options = {}) {
    const wornArmor = this.armorOpposedTransformation(
      actor,
      actor.items.filter(
        (x) => x.type == 'armor' && x.system.worn.value == true,
      ),
      options,
    );
    const protection = wornArmor.reduce(
      (a, b) =>
        a + EquipmentDamage.armorWearModifier(b, b.system.protection.value),
      0,
    );
    const animalArmor = actor.items.reduce(
      (a, b) =>
        a +
        (b.type == 'trait' && b.system.traitType.value == 'armor'
          ? Number(b.system.at.value)
          : 0),
      0,
    );
  ...
  static _calculateCombatSkillValues(
    skill,
    actorData,
    { step, parry, attack } = { step: 0, parry: 0, attack: 0 },
  ) {
    ...
    if (skill.system.weapontype.value == 'melee') {
      ...
      skill.system.parry.value =
        Math.ceil(modifiedTalentValue / 2) +
        Math.max(0, Math.floor((parryChar - 8) / 3)) +
        Number(game.settings.get('dsa5', 'higherDefense')) +
        parry;
      skill.system.attack.value =
        modifiedTalentValue +
        Math.max(0, Math.floor((attackChar - 8) / 3)) +
        attack;
    } else {
      const attackChar =
  ...
  _onUpdateDescendantDocuments(...args) {
    ...
    const force =
      ...
      args[3].some((x) => {
        return [
          'flags.dsa5.auraRadius',
          'flags.dsa5.borderColor',
          'flags.dsa5.disposition',
          'flags.dsa5.fillColor',
          'flags.dsa5.borderThickness',
        ].some((y) => hasProperty(x, y));
      });
  ...
  _perpareItemAdvancementCost(item) {
    const category =
      this.system.isPet || this.system.isFamiliar ? 'C' : item.system.StF.value;
    item.cost = game.i18n.format('advancementCost', {
      cost: DSA5_Utility._calculateAdvCost(
        item.system.talentValue.value,
        category,
      ),
    });
  ...
  schipshtml() {
    ...
    for (let i = 1; i <= Number(this.system.status.fatePoints.max); i++) {
      schips.push({
        value: i,
        cssClass:
          i <= Number(this.system.status.fatePoints.value)
            ? 'fullSchip'
            : 'emptySchip',
      });
    }
  ...
  prepareItems(sheetInfo) {
    ...
    const specAbs = Object.fromEntries(
      Object.keys(DSA5.specialAbilityCategories).map((x) => [x, []]),
    );
    const traits = Object.fromEntries(
      Object.keys(DSA5.traitCategories).map((x) => [x, []]),
    );
    ...
    let containers = new Map();
    for (let container of actorData.items.filter(
      (x) => x.type == 'equipment' && x.system.equipmentType.value == 'bags',
    )) {
    ...
    let hasTrait = false;
    const hasAnyItem = actorData.items.some(
      (x) => !['skill', 'combatskill', 'money'].includes(x.type),
    );
    ...
    for (let i of actorData.items) {
      try {
        ...
        if (i.type == 'ammunition')
          availableAmmunition.push(Actordsa5._prepareitemStructure(i));
        ...
        if (sheetInfo.details && sheetInfo.details.includes(i._id))
          i.detailed = 'shown';
        ...
        switch (i.type) {
          case 'skill':
            skills[i.system.group.value].push(
              this._perpareItemAdvancementCost(i),
            );
            break;
          case 'information':
            information.push(i);
          ...
          case 'ceremony':
            magic[i.type].push(
              Actordsa5.buildSpellChargeProgress(
                this._perpareItemAdvancementCost(i),
              ),
            );
            break;
          ...
          case 'combatskill':
            combatskills.push(
              Actordsa5._calculateCombatSkillValues(i, this.system),
            );
            break;
          case 'ammunition':
          ...
          case 'meleeweapon':
            ...
            inventory.meleeweapons.items.push(
              Actordsa5._prepareitemStructure(i),
            );
          ...
          case 'rangeweapon':
            ...
            i.toggle = true;
            this._setOnUseEffect(i);
            inventory.rangeweapons.items.push(
              Actordsa5._prepareitemStructure(i),
            );
          ...
          case 'armor':
            ...
            if (i.system.worn.value) {
              for (let property in i.system.protection) {
                ...
                i.system.protection[property] =
                  EquipmentDamage.armorWearModifier(i, value);
              }
          ...
          case 'plant':
            ...
            break;
          case 'consumable':
            inventory[i.system.equipmentType.value].items.push(
              Actordsa5._prepareConsumable(i),
            );
      ...
      try {
        meleeweapons.push(
          Actordsa5._prepareMeleeWeapon(
            ...
            actorData,
            wornweapons.filter(
              (x) => x._id != wep._id && !RuleChaos.isYieldedTwohanded(x),
            ),
          ),
        );
      } catch (error) {
    ...
    money.coins = money.coins.sort((a, b) =>
      a.system.price.value > b.system.price.value ? -1 : 1,
    );
    ...
    for (let traditionAbility of specAbs.staff) {
      const artifact = traditionArtifacts.find(
        (x) => x.system.artifact == traditionAbility.system.artifact,
      );
      if (artifact) {
        ...
        artifact[volAttr] +=
          Math.abs(vol) * Number(traditionAbility.system.step.value);
      } else {
    ...
    const brawling = combatskills.find(
    ...
    return {
      ...
      diseases,
      canBuild:
        game.dsa5.sheets.DSACharBuilder &&
        !actorData.system.details.species?.value,
      itemModifiers: this.system.itemModifiers,
      languagePoints: actorData.system.freeLanguagePoints?.value
        ? `<span data-tooltip="languagePoints">(${actorData.system.freeLanguagePoints?.used}/${actorData.system.freeLanguagePoints?.value})</span>`
        : '',
      schips,
      groupschips,
      guidevalues,
  ...
  getArmorEncumbrance(actorData, wornArmors) {
    const encumbrance = wornArmors.reduce((sum, a) => {
      a.system.calculatedEncumbrance =
        Number(a.system.encumbrance.value) +
        EquipmentDamage.armorEncumbranceModifier(a);
  ...
  _calcBagweight(elem, containers, topLevel = true) {
    ...
    if (containers.has(elem._id)) {
      let bagweight = 0;
      if (!elem.system.worn.value && topLevel)
        totalWeight -= elem.system.preparedWeight;

      for (let child of containers.get(elem._id)) {
        child.system.preparedWeight = Number(
          parseFloat(
            (child.system.weight.value * child.system.quantity.value).toFixed(
              3,
            ),
          ),
        );
  ...
  _setBagContent(elem, containers) {
    if (containers.has(elem._id)) {
      ...
      for (let child of containers.get(elem._id)) {
        elem.children.push(
          Actordsa5._prepareitemStructure(Actordsa5._prepareConsumable(child)),
        );
        if (containers.has(child._id)) {
  ...
  isMerchant() {
    return ['merchant', 'loot'].includes(
      getProperty(this, 'system.merchant.merchantType'),
    );
  }

  _itemPreparationError(item, error) {
    console.error(
      'Something went wrong with preparing item ' + item.name + ': ' + error,
    );
    console.warn(error);
    console.warn(item);
    ui.notifications.error(
  ...
  _applyModiferTransformations(itemModifiers) {
    ...
    for (const key of Object.keys(itemModifiers)) {
      ...
      if (shortCut) {
        const modSum = itemModifiers[key].reduce(
          (prev, cur) => (prev = prev + cur.value),
          0,
        );

        this.system[shortCut[0]][shortCut[1]][shortCut[2]] += modSum;

        this.system.itemModifiers[key] = {
          value: modSum,
      ...
      if (vals.length == 2) {
        if (!isNaN(vals[0])) {
          let elem = {
            value:
              Number(vals[0]) *
              (i.system.step ? Number(i.system.step.value) || 1 : 1),
            source: i.name,
            type: i.type,
          };
  ...
  async _updateAPs(APValue, dataUpdate = {}, options = {}) {
    if (Actordsa5.canAdvance(this)) {
      if (!isNaN(APValue) && !(APValue == null)) {
        ...
        dataUpdate['system.details.experience.spent'] =
          Number(this.system.details.experience.spent) + ap;
        await this.update(dataUpdate, options);
        const msg = game.i18n.format(
          ap > 0 ? 'advancementCost' : 'refundCost',
          { cost: Math.abs(ap) },
        );
  ...
  async checkEnoughXP(cost) {
    ...
    if (
      Number(this.system.details.experience.total) -
        Number(this.system.details.experience.spent) >=
      cost
    ) {
      return true;
  ...
  setupWeapon(item, mode, options, tokenId) {
    options['mode'] = mode;
    return Itemdsa5.getSubClass(item.type).setupDialog(
      null,
      options,
      item,
      this,
  ...
  throwMelee(item, tokenId) {
    ...
    const options = {
      situationalModifiers: [
        { name, value: hasWeaponThrow ? -4 : -8, selected: true },
      ],
    };

    this.setupWeapon(range, 'attack', options, tokenId).then(
      async (setupData) => {
        if (!hasWeaponThrow) {
  ...
  setupWeaponless(statusId, options = {}, tokenId) {
    ...
    if (
      SpecialabilityRulesDSA5.hasAbility(
        this,
        game.i18n.localize('LocalizedIDs.mightyAstralBody'),
      )
    )
    ...
    if (
      SpecialabilityRulesDSA5.hasAbility(
        this,
        game.i18n.localize('LocalizedIDs.mightyKarmalBody'),
      )
    )
  ...
  setupSkill(skill, options = {}, tokenId) {
    return Itemdsa5.getSubClass(skill.type).setupDialog(
      null,
      options,
      skill,
      this,
  ...
  _containsChangedAttribute(data, key) {
    ...
    return [null, undefined].includes(newValue) ||
      newValue === getProperty(this, key)
      ? false
      : newValue;
  }
  ...
  async _preUpdate(data, options, user) {
    ...
    for (let key of Object.keys(statusText)) {
      const value = this._containsChangedAttribute(
        data,
    ...
    const swarmCount = this._containsChangedAttribute(
      data,
      'system.swarm.count',
    );
    if (swarmCount !== false && !options.skipSwarmUpdate) {
      const hp =
        getProperty(data, 'system.status.wounds.value') ||
        this.system.status.wounds.value;
      const delta = swarmCount - (this.system.swarm.count || 1);
      const baseHp =
        this.system.swarm.maxwounds || this.system.status.wounds.max;
      setProperty(
        data,
        'system.status.wounds.value',
        Math.max(0, hp + delta * baseHp),
      );
    }

    const apSum = this._containsChangedAttribute(
      data,
  ...
  async applyDamage(rollFormula, options = {}) {
    ...
    if (game.combat?.isBrawling) {
      const newVal = Math.min(
        this.system.status.temporaryLeP.max,
        this.system.status.temporaryLeP.value - amount,
      );
      await this.update({ 'system.status.temporaryLeP.value': newVal });
    } else {
      const newVal = Math.min(
        this.system.status.wounds.max,
        this.system.status.wounds.value - amount,
      );
    ...
    if (options.msg) {
      ...
      ChatMessage.create(
        DSA5_Utility.chatDataSetup(
          `<p>${game.i18n.format(options.msg, { name: this.name })}</p>${renderedRoll}`,
        ),
      );
    }
  }
  ...
  async applyRegeneration(LeP, AsP, KaP) {
    const update = {
      'system.status.wounds.value': Math.min(
        this.system.status.wounds.max,
        this.system.status.wounds.value + (LeP || 0),
      ),
  ...
  async applyMana(rollFormula, type) {
    ...
    const newVal = Math.min(
      this.system.status[state].max,
      this.system.status[state].value - amount,
    );
    if (newVal >= 0) {
  ...
  async fatererollDamage(
    ...
    let newRoll = await DiceDSA5.manualRolls(
      await new Roll(
        oldDamageRoll.formula || oldDamageRoll._formula,
      ).evaluate(),
  ...
  async fateisTalented(infoMsg, cardOptions, newTestData, message, data) {
    ...
    const html = await renderTemplate(
      'systems/dsa5/templates/dialog/isTalentedReroll-dialog.html',
      {
            ...
            if (diesToReroll.length > 0) {
              ...
              for (let k of diesToReroll) {
                const characteristic =
                  newTestData.source.system[`characteristic${k + 1}`];
                const attr = characteristic
                  ? game.i18n.localize(
                      `CHARAbbrev.${characteristic.value.toUpperCase()}`,
                    ) + ' - '
                  : '';
  ...
  async fatereroll(
    ...
    const html = await renderTemplate(
      'systems/dsa5/templates/dialog/fateReroll-dialog.html',
      {
        testData: newTestData,
        postData: data.postData,
        ...
        Yes: {
          ...
          callback: async (dlg) => {
            ...
            if (diesToReroll.length > 0) {
              ...
              const phexTradition = game.i18n.localize(
                'LocalizedIDs.traditionPhex',
              );
              const isPhex = actor.items.some(
                (x) => x.type == 'specialability' && x.name == phexTradition,
              );
  ...
  async fateImprove(
    ...
    if (
      ['spell', 'liturgy', 'ceremony', 'ritual', 'skill'].includes(rollType)
    ) {
      const html = await renderTemplate(
        'systems/dsa5/templates/dialog/fateImprove-dialog.html',
        {
          testData: newTestData,
          postData: data.postData,
        },
          ...
          Yes: {
            ...
            callback: async (dlg) => {
              ...
              if (diesToUpgrade.length == 1) {
                ...
                newTestData.roll.terms[diesToUpgrade * 2].results[0].result =
                  Math.max(
                    1,
                    newTestData.roll.terms[diesToUpgrade * 2].results[0]
                      .result - 2,
                  );
      ...
      newTestData.situationalModifiers.push(modifier);
      newTestData.roll.terms[0].results[0].result = Math.max(
        1,
        newTestData.roll.terms[0].results[0].result - 2,
      );
      this[`${data.postData.postFunction}`](
        { testData: newTestData, cardOptions },
        { rerenderMessage: message },
  ...
  async reduceSchips(schipsource) {
    if (schipsource == 0)
      await this.update({
        'system.status.fatePoints.value':
          this.system.status.fatePoints.value - 1,
      });
    else {
  ...
  async useFateOnRoll(message, type, schipsource) {
    if (
      type == 'isTalented' ||
      DSA5_Utility.fateAvailable(this, schipsource == 1)
    ) {
      let data = message.flags.data;
      ...
      newTestData.extra.actor = DSA5_Utility.getSpeaker(
        newTestData.extra.speaker,
      ).toObject(false);
  ...
  setupFallingDamage(options, tokenId) {
    ...
    const skill = this.items
      .find(
        (x) =>
          x.type == 'skill' &&
          x.name == game.i18n.localize('LocalizedIDs.bodyControl'),
      )
      .toObject();
    const optns = {
      subtitle: ` (${name})`,
    ...
    this.setupSkill(skill, optns, tokenId).then(async (finalData) => {
      ...
      await DiceDSA5.renderRollCard(
        res.cardOptions,
        res.result,
        res.options.rerenderMessage,
      );
    });
  }
  ...
  static async updateFallingDamage(postFunction, result, source) {
    ...
    const setupData = await actor._setupFallingHeight(
      postFunction.options,
      postFunction.tokenId,
    );
    const fallingDamage = await actor.basicTest(setupData, {
      suppressMessage: true,
    });
    const html = await renderTemplate(
      'systems/dsa5/templates/chat/roll/fallingdamage-card.html',
      fallingDamage,
    );
  ...
  _setupFallingHeight(options, tokenId) {
    ...
    const cardOptions = this._setupCardOptions(
      'systems/dsa5/templates/chat/roll/fallingdamage-card.html',
      title,
      tokenId,
    );
  ...
  setupRegeneration(statusId, options = {}, tokenId) {
    ...
    let dialogOptions = {
      ...
      callback: (html, options = {}) => {
        ...
        testData.situationalModifiers.push(
          {
            name:
              ...
              ' - ' +
              html
                .find('[name="regnerationCampLocations"] option:selected')
                .text(),
          ...
          },
          {
            name:
              ...
              ' - ' +
              html
                .find('[name="regenerationInterruptOptions"] option:selected')
                .text(),
            value: html.find('[name="regenerationInterruptOptions"]').val(),
          },
        );
        testData.regenerationFactor = html
          .find('[name="badEnvironment"]')
          .is(':checked')
          ? 0.5
          : 1;
        ...
        const update = {};
        for (let k of attrs) {
          testData[`${k}Modifier`] = Number(
            html.find(`[name="${k}Modifier"]`).val() || 0,
          );
          testData[`regeneration${k}`] = Number(
            this.system.status.regeneration[`${k}max`],
          );
          const regenerate = html.find(`[name="regenerate${k}"]`).is(':checked')
            ? 1
            : 0;
    ...
    let cardOptions = this._setupCardOptions(
      'systems/dsa5/templates/chat/roll/regeneration-card.html',
      title,
      tokenId,
    );
  ...
  setupDodge(options = {}, tokenId) {
    ...
    const toSearch = [
      game.i18n.localize(statusId),
      game.i18n.localize('LocalizedIDs.wrestle'),
    ];
    const combatskills = [
      ...Itemdsa5.buildCombatSpecAbs(
        this,
        ['Combat'],
        toSearch,
        'parry',
    ...
    const situationalModifiers = DSA5StatusEffects.getRollModifiers(
      testData.extra.actor,
      testData.source,
    );
    ...
    const dialogOptions = {
      ...
      template:
        'systems/dsa5/templates/dialog/combatskill-enhanced-dialog.html',
      data,
      callback: (html, options = {}) => {
    ...
    const cardOptions = this._setupCardOptions(
      'systems/dsa5/templates/chat/roll/status-card.html',
      dialogOptions.title,
      tokenId,
    );
  ...
  setupCharacteristic(characteristicId, options = {}, tokenId) {
    ...
    let title =
      DSA5_Utility.attributeLocalization(characteristicId) +
      ' ' +
      game.i18n.localize('Test');
      ...
      callback: (html, options = {}) => {
        ...
        testData.testDifficulty =
          DSA5.attributeDifficultyModifiers[
            html.find('[name="testDifficulty"]').val()
          ];
        testData.situationalModifiers = Actordsa5._parseModifiers(html);
        mergeObject(testData.extra.options, options);
        return { testData, cardOptions };
    ...
    let cardOptions = this._setupCardOptions(
      'systems/dsa5/templates/chat/roll/characteristic-card.html',
      title,
      tokenId,
    );
  ...
  static _prepareitemStructure(item) {
    ...
    if (enchants && enchants.length > 0) {
      ...
      if (item.type == 'armor') {
        for (let mod of item.system.effect.value
          .split(/,|;/)
          .map((x) => x.trim())) {
          let vals = mod.replace(/(\s+)/g, ' ').trim().split(' ');
  ...
  static _prepareMeleeWeapon(
    ...
    let skill = combatskills.find(
      (i) => i.name == item.system.combatskill.value,
    );
    if (skill) {
      item.attack =
        Number(skill.system.attack.value) + Number(item.system.atmod.value);
      ...
      item.parry =
        ...
        (RuleChaos.isShield(item) ? Number(item.system.pamod.value) : 0);
      ...
      if (!item.yieldedTwoHand) {
        ...
        if (wornWeapons.length > 0) {
          item.parry += Math.max(
            ...wornWeapons.map((x) => x.system.pamod.offhandMod),
          );
          item.attack += Math.max(
            ...wornWeapons.map((x) => x.system.atmod.offhandMod),
          );
        }
      }
          ...
          switch (localizedCT) {
            ...
            case 'Two-Handed Swords':
              ...
              const reg = new RegExp(
                game.i18n.localize('wrongGrip.wrongGripBastardRegex'),
              );
      ...
      if (item.system.guidevalue.value != '-') {
        let val = Math.max(
          ...item.system.guidevalue.value
            .split('/')
            .map((x) => Number(actorData.system.characteristics[x].value)),
        );
        let extra =
          Math.max(val - Number(item.system.damageThreshold.value), 0) +
          gripDamageMod;
        if (extra > 0) {
          item.extraDamage = extra;
          item.damageAdd = Roll.safeEval(
            item.damageAdd + ' + ' + Number(extra),
          );
      ...
      if (isBaseWeapon) {
        ...
        for (let key of Object.keys(
          getProperty(item, 'flags.dsa5.alternateAttacks') || {},
        )) {
          const dup = this.buildSubweapon(item, key);
          const done = this._prepareMeleeWeapon(
            dup,
            combatskills,
            actorData,
            wornWeapons,
  ...
  static buildSubweapon(item, id) {
    ...
    for (let key of Object.keys(data)) {
      if (
        this.skipAlternateWeaponKeys.has(data[key]) ||
        data[key] == null ||
        data[key] == undefined
      )
        delete data[key];
  ...
  async actorEffects() {
    ...
    return isAllowedToSeeEffects
      ? this.effects.filter((x) => x.isVisibleEffect())
      : this.effects.filter((x) =>
          allowedEffects.some((y) => x.statuses.has(y)),
        );
  }
    ...
    switch (item.type) {
      ...
      case 'rangeweapon':
        const items = this.items.filter(
          (x) => x.type == item.type && x.id != itemId && x.system.worn.value,
        );
        updates = items.map((x) => {
  ...
  static calcLZ(item, actor) {
    ...
    let modifier = 0;
    if (
      item.system.combatskill.value ==
      game.i18n.localize('LocalizedIDs.Throwing Weapons')
    )
      modifier =
        SpecialabilityRulesDSA5.abilityStep(
          actor,
          game.i18n.localize('LocalizedIDs.quickdraw'),
        ) * -1;
    else if (
      item.system.combatskill.value ==
        game.i18n.localize('LocalizedIDs.Crossbows') &&
      SpecialabilityRulesDSA5.hasAbility(
        actor,
    ...
    if (item.system.ammunitiongroup.value == 'mag') {
      let currentAmmo = actor.items.find(
        (x) =>
          x.id == item.system.currentAmmo.value ||
          x._id == item.system.currentAmmo.value,
      );
      let reloadType = 0;
      if (currentAmmo) {
  ...
  static _parseDmg(item, rollData, modification = undefined) {
    const parseDamage = new Roll(
      DiceDSA5.replaceDieLocalization(item.system.damage.value),
      rollData || {},
    );
    ...
    item.damageAdd =
      damageTerm != '' ? (Number(damageTerm) >= 0 ? '+' : '') + damageTerm : '';
  ...
  static _prepareRangeWeapon(
    ...
    if (skill) {
      ...
      if (item.system.ammunitiongroup.value != '-') {
        item.ammo = ammunitions.filter(
          (x) =>
            x.system.ammunitiongroup.value == item.system.ammunitiongroup.value,
        );

        for (let am of item.ammo)
          am.label = `(${am.system.quantity.value}) ${am.name}`;

        currentAmmo = item.ammo.find(
          (x) => x._id == item.system.currentAmmo.value,
        );
        if (currentAmmo) {
          const rangeMultiplier =
            Number(currentAmmo.system.rangeMultiplier) || 1;
      ...
      if (isBaseWeapon) {
        ...
        for (let key of Object.keys(
          ...
          const dup = this.buildSubweapon(item, key);
          const done = this._prepareRangeWeapon(
            dup,
            ammunitions,
            combatskills,
            actor,
  ...
  _setupCardOptions(template, title, tokenId) {
    ...
    if (this.token) {
      ...
      if (speaker.actor == this.id) {
        ...
        cardOptions.flags.img = speaker.token
          ? canvas.tokens.get(speaker.token).img
          : cardOptions.flags.img;
      }
    }
    return cardOptions;
  }
  ...
  async toggleStatusEffect(statusId, { active, overlay = false } = {}) {
    ...
    if (overlay) {
      ...
      if (
        !existing ||
        Number.isNumeric(getProperty(existing, 'flags.dsa5.value'))
      ) {
  ...
  async payMiracles(testData) {
    if (!testData.extra.miraclePaid) {
      ...
      const hasMiracleMight = testData.situationalModifiers.some(
        (x) => x.name.trim() == miracleMight,
      );
      const hasMiracle = testData.situationalModifiers.some(
        (x) => x.name.trim() == miracle,
      );
      const cost = hasMiracleMight ? 6 : hasMiracle ? 4 : 0;
      if (cost) {
        await this.update({
          'system.status.karmaenergy.value':
            this.system.status.karmaenergy.value - cost,
        });
      }
    }
  }
  ...
  async consumeAmmunition(testData) {
    if (testData.extra.ammo && !testData.extra.ammoDecreased) {
      ...
      if (testData.extra.ammo._id) {
        ...
        if (testData.extra.ammo.system.ammunitiongroup.value == 'mag') {
          if (testData.extra.ammo.system.mag.value <= 0) {
            ...
            ammoUpdate['system.quantity.value'] =
              testData.extra.ammo.system.quantity.value;
            ammoUpdate['system.mag.value'] =
              testData.extra.ammo.system.mag.max - 1;
          } else {
            ammoUpdate['system.mag.value'] =
              testData.extra.ammo.system.mag.value - 1;
          }
        } else {
          testData.extra.ammo.system.quantity.value--;
          ammoUpdate['system.quantity.value'] =
        ...
        await this.updateEmbeddedDocuments('Item', [
          ammoUpdate,
          { _id: testData.source._id, 'system.reloadTime.progress': 0 },
        ]);
      }
    } else if (
      ...
      ['spell', 'liturgy'].includes(testData.source.type) &&
      testData.extra.speaker.token != 'emptyActor'
    ) {
      await this.updateEmbeddedDocuments('Item', [
        {
    ...
    switch (item.type) {
      case 'combatskill':
        max =
          Math.max(
            ...item.system.guidevalue.value
              .split('/')
              .map((x) => this.system.characteristics[x].value),
          ) +
          2 +
          maxBonus;
        break;
      ...
      case 'ritual':
        ...
        for (const feature of item.system.feature
          ...
          if (
            SpecialabilityRulesDSA5.hasAbility(
              this,
              `${game.i18n.localize('LocalizedIDs.propertyKnowledge')} (${feature})`,
            )
          ) {
      ...
      case 'ceremony':
        const aspect = new RegExp(
          `^${game.i18n.localize('LocalizedIDs.aspectKnowledge')}`,
        );
        let aspectValue = 0;
        if (
          this.items
            ...
            .some((x) =>
              item.system.distribution.value.includes(
  ...
  async basicTest({ testData, cardOptions }, options = {}) {
    ...
    if (game.user.targets.size) {
      ...
      if (
        cardOptions.isOpposedTest &&
        cardOptions.title.match(opposed + '$') != opposed
      )
    ...
    if (!options.suppressMessage) {
      const msg = await DiceDSA5.renderRollCard(
        cardOptions,
        result,
        options.rerenderMessage,
      );
  ...
  async addCondition(effect, value = 1, absolute = false, auto = false) {
    if (effect == 'bleeding' || effect.id == 'bleeding')
    ...
    return await DSA5StatusEffects.addCondition(
      this,
      effect,
      value,
      absolute,
  ...
  async initResistPainRoll(effect) {
    ...
    const template = await renderTemplate(
      'systems/dsa5/templates/chat/roll/resist-pain.html',
      { actor: this },
    );
  ...
  async finishResistPainRoll() {
    const skill = this.items.find(
      (x) =>
        x.name == game.i18n.localize('LocalizedIDs.selfControl') &&
        x.type == 'skill',
    );
    this.setupSkill(
      skill,
      { subtitle: ` (${game.i18n.localize('ActiveEffects.resistRoll')})` },
      this.token?.id,
  ...
  async removeCondition(effect, value = 1, auto = true, absolute = false) {
    return await DSA5StatusEffects.removeCondition(
      this,
      effect,
      value,
      auto,
