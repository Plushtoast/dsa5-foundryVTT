...
export default class DSAActiveEffectConfig extends ActiveEffectConfig {
  ...
  async _render(force = false, options = {}) {
    ...
    if (effectConfigs.hasDamageTransformation) {
      advancedFunctions.push(
        { name: "ActiveEffects.advancedFunctions.armorPostprocess", index: 4 },
        { name: "ActiveEffects.advancedFunctions.damagePostprocess", index: 5 }
      );
    }
    const config = {
  ...
  static async applyAdvancedFunction(actor, effects, source, testData, sourceActor, skipResistRolls = true) {
    ...
    for (const ef of effects) {
      try {
        ...
        if (resistRoll && !skipResistRolls) {
          ...
          resistRolls.push({
            skill: skills.join(" "),
            mod: Roll.safeEval(mod.replace(/q(l|s)/i, qs)) || 0,
            effect: ef,
            target: actor,
    ...
    return { msg, resistRolls, effectApplied };
  }
  ...
  static async applyEffect(id, mode, targets, options = {}) {
    ...
    if (game.user.isGM) {
      for (let actor of actors) {
        const { msg, resistRolls, effectApplied } = await DSAActiveEffectConfig.applyAdvancedFunction(
          actor,
          effects,
          source,
          testData,
        ...
        if (effectApplied) {
          const appliedEffect = game.i18n.format("ActiveEffects.appliedEffect", { target: actor.name, source: source.name });
  ...
  dropDownMenu() {
    ...
    for (const k of models) {
      ...
      optns.push(
        { name: `${el} - ${FW}`, val: `data.skillModifiers.${k}.FW`, mode: 0, ph: demo },
        { name: `${el} - ${FP}`, val: `data.skillModifiers.${k}.FP`, mode: 0, ph: demo },
        { name: `${el} - ${stepValue}`, val: `data.skillModifiers.${k}.step`, mode: 0, ph: demo },
        { name: `${el} - ${QS}`, val: `data.skillModifiers.${k}.QL`, mode: 0, ph: demo },
        { name: `${el} - ${partChecks}`, val: `data.skillModifiers.${k}.TPM`, mode: 0, ph: demo }
      );
    }
