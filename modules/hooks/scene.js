...
export default function () {
  Hooks.on('preCreateScene', function (doc, createData, options, userId) {
    if (!createData.grid?.units)
      doc.updateSource({ grid: { units: game.i18n.localize('gridUnits') } });
    ...
    if (
      !doc.pack &&
      !options.dsaInit &&
      createData.notes?.some((x) => getProperty(x, 'flags.dsa5.initName'))
    ) {
          ...
          },
          {
            ...
            callback: async () => {
              try {
                ...
                const initializer = new game.dsa5.apps.DSA5Initializer(
  ...
  Hooks.on(
    ...
    function (doc, createData, options, userId) {
