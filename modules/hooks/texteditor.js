...
export function setEnrichers() {
  ...
  const titles = {
    Rq: '',
    Gc: `${game.i18n.localize('HELP.groupcheck')} `,
    Ch: '',
    AP: '',
    Pay: '',
    GetPaid: '',
    ...
    },
    {
      ...
      enricher: async (match, options) => {
        ...
        const enriched = {
          ...
          enrichedqs2: await TextEditor.enrichHTML(item.system.qs2, {
            async: true,
          }),
          enrichedqs3: await TextEditor.enrichHTML(item.system.qs3, {
            async: true,
          }),
          enrichedqs4: await TextEditor.enrichHTML(item.system.qs4, {
            async: true,
          }),
          enrichedqs5: await TextEditor.enrichHTML(item.system.qs5, {
            async: true,
          }),
          enrichedqs6: await TextEditor.enrichHTML(item.system.qs6, {
            async: true,
          }),
          enrichedCrit: await TextEditor.enrichHTML(item.system.crit, {
            async: true,
          }),
          enrichedBotch: await TextEditor.enrichHTML(item.system.botch, {
            async: true,
          }),
          enrichedFail: await TextEditor.enrichHTML(item.system.fail, {
            async: true,
          }),
        };
