...
export default class DiceDSA5 {
  ...
  static #buildAttributesList() {
    return Object.keys(DSA5.characteristics).reduce((acc, attr) => {
      acc[attr] = game.i18n.localize(`CHARAbbrev.${attr.toUpperCase()}`);
      return acc;
    }, {});
  }
  ...
  static #adjustForOpportunityAttack(testData, adjustThresholds) {
    const opportunityAttack = testData.situationalModifiers.find(
      (x) => x.name === game.i18n.localize('MODS.opportunityAttack') && x.value !== 0
    );
  ...
  static async #handleConfirmationRoll(isCrit, isBotch, adjustedRes, combatskill, actor, testData, id) {
    ...
    const baseDescription = game.i18n.localize(isCrit ? 'CriticalSuccess' : 'CriticalFailure');
    ...
    // Perform confirmation roll
    const confirmationResult = await this.#performConfirmationRoll(
      isCrit, isBotch, adjustedRes, combatskill, actor, testData, id
    );

    return {
      description: `${game.i18n.localize(confirmationResult.confirmed ? 'confirmed' : 'unconfirmed')} ${baseDescription}${confirmationResult.additionalDescription}`,
  ...
  static async #performConfirmationRoll(isCrit, isBotch, adjustedRes, combatskill, actor, testData, id) {
    ...
    // Handle weapon aptitude for failed confirmation
    if (this.#shouldUseWeaponAptitude(actor, combatskill, confirmResult)) {
      ...
      additionalDescription = `, ${game.i18n.format('usedWeaponExpertise', { a: oldRoll, b: rollConfirm.total })}`;
    }
  ...
  static #buildSingleD20Result(rollResult, confirmationResult, testData, modifier, isCrit, isBotch) {
    ...
    // Use confirmation result if available
    if (confirmationResult.successLevel !== null) {
      ...
      description = game.i18n.localize(rollResult.success ? 'Success' : 'Failure');
    }
  ...
  static async _stringToRoll(text, testData) {
    try {
      ...
      const dicePattern = DICE_CONSTANTS.PATTERNS.DICE_NOTATION;
      let match;
      ...
      // If no dice found, try to evaluate as number
      if (diceMatches.length === 0) {
        return Number(text) || 0;
      }

      // Evaluate all dice rolls
      ...
      // Add dice animations if testData provided
      if (testData) {
      ...
      // Replace dice notation with results and evaluate
      let evaluationString = modifiedText;
      rollResults.forEach(roll => {
        // Create a new RegExp instance to avoid state issues with global regex
  ...
  static replaceDieLocalization(formula) {
    return formula.replace(DICE_CONSTANTS.PATTERNS.DIE_LOCALIZATION, 'd');
  }
  ...
  static async _rollThreeD20(testData) {
    ...
    if (testData.source.type == SKILL && AdvantageRulesDSA5.hasVantage(actor, `${game.i18n.localize('LocalizedIDs.incompetent')} (${testData.source.name})`, false)) {
    ...
    if (testData.source.type == SKILL && TraitRulesDSA5.hasTrait(actor, `${game.i18n.localize('LocalizedIDs.automaticSuccess')} (${testData.source.name})`, false)) {
      description.push(game.i18n.localize('LocalizedIDs.automaticSuccess'));
      successLevel = 1;
      automaticResult = 1;
    } else if (testData.source.type == SKILL && TraitRulesDSA5.hasTrait(actor, `${game.i18n.localize('LocalizedIDs.automaticFail')} (${testData.source.name})`, false)) {
  ...
  static get3D20SuccessLevel(
    ...
    // Count critical and botch results
    ...
    // Multiple criticals/botches override skill success
    if (criticalCount >= 2) return criticalCount;
    if (botchCount >= 2) return botchCount * -1;

    // Single success/failure based on remaining skill points
    return fws >= 0 ? 1 : -1;
  }
