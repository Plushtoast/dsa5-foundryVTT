...
export default class ItemRulesDSA5 {
  ...
  static getTalentBonus(actor, talent, types) {
    const modifier = [];
    const selected = game.settings.get('dsa5', 'talentModifierEnabled');
    for (const k of actor.items.filter((x) => {
      return types.includes(x.type) && `${x.system.effect.value}`.includes(talent);
    })) {
      for (const m of k.system.effect.value.split(/;|,/)) {
        if (m.includes(talent)) {
          const parsed = DSA5_Utility.parseAbilityString(m.trim());
  ...
  static reverseAdoptionCalculation(actor, parsed, item) {
    ...
    for (const elem of elems) {
      if (elem[parsed.name]) {
        const adoption = actor.items.find((x) => elem[parsed.name].items.includes(x.type) && x.name == parsed.special);
        if (adoption) {
  ...
  static itemStep(actorData, name, types) {
    const item = actorData.items.find((x) => types.includes(x.type) && x.name == name);
    if (item) {
