export class MeasuredTemplateDSA extends MeasuredTemplate {
    ...
    static fromItem(item, qs) {
        ...
        const distance = Number(`${target.value}`.replace(/(qs|ql)/gi, qs)) || 1
        const templateData = {
            t: templateShape,
            user: game.user.id,
            distance,
        ...
        switch (templateShape) {
            ...
            case "rect":
                ...
                break;
            case "ray":
                templateData.width = target.width ? (Number(`${target.width}`.replace(/(qs|ql)/gi, qs)) || canvas.dimensions.distance) : canvas.dimensions.distance;
                break;
        }
