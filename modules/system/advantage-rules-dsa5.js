...
export default class AdvantageRulesDSA5 {
    ...
    static async needsAdoption(actor, item, typeClass) {
        if (DSA5.vantagesNeedingAdaption[item.name]) {
            ...
            let template = await renderTemplate('systems/dsa5/templates/dialog/requires-adoption-dialog.html', { items: items, original: item })
            await new Dialog({
    ...
    static hasVantage(actor, talent) {
        return actor.items.find(x => (x.type == "advantage" || x.type == "disadvantage") && x.name == talent) != undefined
    }
    static vantageStep(actor, talent) {
        let item = actor.items.find(x => (x.type == "advantage" || x.type == "disadvantage") && x.name == talent)
        if (item) {
    ...
    static getVantageAsModifier(actor, talent, factor = 1) {
        let res = []
        let item = actor.items.find(x => (x.type == "advantage" || x.type == "disadvantage") && x.name.match(talent))
        if (item) {
            res.push({
                name: item.name,
    ...
    static getTalentBonus(actor, talent) {
        let modifier = []
        for (let k of actor.items.filter(x => { return (x.type == "advantage" || x.type == "disadvantage") && x.data.effect.value.includes(talent) })) {
            for (let m of k.data.effect.value.split(";")) {
                if (m.includes(talent)) {
                    let vals = m.split(" ")
                    let value = Number(vals.pop())
                    if (vals.join(" ") == talent) {
                        modifier.push({
                            name: k.name,
                            value: value * k.data.step.value
                        })
                    }
                }
            }
